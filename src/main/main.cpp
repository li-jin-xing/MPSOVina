...
#include "parallel_mpso.h" //MPSOVina
#include "file.h"
#include "cache.h"
#include "non_cache.h"
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   const parallel_pso& par/*MPSOVina*/, fl energy_range, sz num_modes, 
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights,int num_birds, int opt, double c1, double c2) {
	...
	if(score_only) {
		...
		refine_structure(m, prec, nc, out, authentic_v, par.pso.ssd_par.evals/*MPSOVina*/);
		done(verbosity, log);
		...
		{
			case 1:
				doing(verbosity, "Performing PSO search", log);
				break;
			case 2:
				doing(verbosity, "Performing QPSO search", log);
				break;
			case 3:
				doing(verbosity, "Performing RDPSO search", log);
				break;
			default:
		...
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_birds, opt, c1, c2);
		done(verbosity, log);
...
if(!out_cont.empty()) {
		...
		VINA_FOR_IN(i, out_cont)
			refine_structure(m, prec, nc, out_cont[i], authentic_v, par.pso.ssd_par.evals/*MPSOVina*/);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, int num_birds,double ratio_steps,int opt,double c1,double c2) {
	...
	parallel_pso par;/*MPSOVina*/
	sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
	par.pso.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2 * ratio_steps); // 2 * 70 -> 8 * 20 // FIXME/*MPSOVina*/
	par.pso.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);/*MPSOVina*/
	par.pso.min_rmsd = 1.0;/*MPSOVina*/
	par.pso.num_saved_mins = 20;/*MPSOVina*/
	par.pso.hunt_cap = vec(10, 10, 10);/*MPSOVina*/
	par.num_tasks = exhaustiveness+1;
	par.num_threads = cpu;
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights,num_birds,opt,c1,c2);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "MPSOVina version 2.0";    //pso
...
Visit http://cbbio.cis.umac.mo for more information.                   \n\
                                                                       \n\
MPSOVina was developed based on the framework of AutoDock Vina.         \n\
                                                                       \n\
...
=======================================================================\n";
	...
	try {
		...
		int cpu = 0, seed, exhaustiveness, verbosity = 2, num_modes = 9,num_birds = 0, opt=1;
		fl energy_range = 2.0;
		...
		;
		options_description pso("QPSO parameters (optional)");
		pso.add_options()
			...
			("opt", value<int>(&opt)->default_value(1,"1(PSO) [2-QPSO,3-RDPSO]"), "Option of running PSO type")
		;
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, num_birds,ratio_steps,opt,c1,c2);
	}
	catch(file_error& e) {
