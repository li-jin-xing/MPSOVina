...
struct parallel_pso_aux {
	...
	const vec* corner1;
	const vec* corner2;
	const int num_birds, opt;
	const double c1,c2;
    const parallel_pso_task_container1 task_container;
	parallel_progress* pg;
	parallel_pso_aux(const parallel_pso_task_container1 task_container_, const pso_search* pso_, const precalculate* p_, const igrid* ig_, const precalculate* p_widened_, const igrid* ig_widened_, const vec* corner1_, const vec* corner2_, parallel_progress* pg_, int num_birds_, int opt_, double c1_, double c2_)
		: task_container(task_container_), pso(pso_), p(p_), ig(ig_), p_widened(p_widened_), ig_widened(ig_widened_), corner1(corner1_), corner2(corner2_), pg(pg_), num_birds(num_birds_), opt(opt_), c1(c1_), c2(c2_) {}
	void operator()(parallel_pso_task& t) const {
		(*pso)(task_container, t.num, t.m, t.out, *p, *ig, *p_widened, *ig_widened, *corner1, *corner2, pg, t.generator, num_birds, opt, c1, c2);
	}
};
...
void parallel_pso::operator()(const model& m, output_container& out, const precalculate& p, const igrid& ig, const precalculate& p_widened, const igrid& ig_widened, const vec& corner1, const vec& corner2, rng& generator, int num_birds, int opt, double c1, double c2) const {
	parallel_progress pp;
    parallel_pso_task_container task_container;
    VINA_FOR(i, num_tasks)
        task_container.push_back(new parallel_pso_task(i, m, random_int(0, 1000000, generator)));
	parallel_pso_aux parallel_pso_aux_instance(task_container, &pso, &p, &ig, &p_widened, &ig_widened, &corner1, &corner2, (display_progress ? (&pp) : NULL), num_birds, opt, c1, c2);
	if(display_progress) 
